...
export default function Home() {
  ...
  async function load() {
    try {
      ...
      const { data } = await supabase.from("goals").select("id,title,description,status,missions(id,title,description,status)").eq("user_id", user.id).order("created_at", { ascending: false });
      const list = (data || []) as Goal[];
      setGoals(list);
      const all = list.flatMap(g => g.missions || []);
      setTotalMissions(all.length); setMissionsDone(all.filter(m => m.status === "completed").length);
  ...
  async function toggleMission(mission: Mission) {
    ...
    await supabase.from("missions").update({ status: next, completed_at: next === "completed" ? new Date().toISOString() : null }).eq("id", mission.id);
    await load();
  }
    ...
    <main className="shell">
      ...
      <section className="content">
        ...
        <section className="grid">
          ...
          <div className="panel goalPanel"><div className="eyebrow">AI GOAL PLANNER</div><h2>What are you trying to make real?</h2><p>Tell LUMA the goal. It will turn it into a practical path.</p><div className="goalInput"><input value={goal} onChange={e=>setGoal(e.target.value)} placeholder="e.g. Launch my first app"/><button onClick={()=>{if(goal.trim())setSubmitted(true)}}>Build my path</button></div>{submitted&&<div className="planReady">✦ Goal captured. AI planning is next. <b>Coming next →</b></div>}</div>
        </section>
